...
export default function App() {
  ...
  const handleViewModeSwitch = useCallback((targetMode: '2D' | '3D') => {
    ...
    if (viewMode === '3D' && targetMode === '2D') {
      // 3D → 2D：读取 Cesium 相机参数，换算 zoom，写入 Store
  ...
  const [messages, setMessages] = useState<ChatMessageType[]>([
    {
      id: 'init-1',
      role: 'user',
      content: '请检索四川省关于国土空间规划中城镇开发边界划定的技术标准和要求。',
      timestamp: new Date().toISOString()
    },
    {
      id: 'init-2',
      role: 'assistant',
      content: '正在分析四川省空间规划数据库... 已定位 3 个高度相关的技术指标。根据《四川省国土空间规划（2021—2035年）》，城镇开发边界划定需遵循以下原则：',
      timestamp: new Date().toISOString(),
      metadata: {
        document_ids: [],
        citations: []
